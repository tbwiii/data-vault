...
import { CodeIcon, EnterIcon, DashboardIcon, FileTextIcon, FilePlusIcon } from '@radix-ui/react-icons'
...
export function Menu() {
    return (
        <div className="flex flex-col gap-2">
            {menuItems.map((item) => (
                <MenuItem key={item.label} label={item.label} icon={item.icon} href={item.href} />
            ))}
        </div>
    )
}
        ...
        >
            ...
            <div>
                <button className="
                    ...
                    type="button"
                >
                    <img src="/data-vault-emblem.png" className="w-10 h-10" alt="Data Valut Emblem" />
                    <span className={`grow text-2xl ${fonts.cutive}`}>{'Data Vault'.toUpperCase()}</span>
                </button>
